...
u8  first_heat_std = 0;
u8  error_std = 0;
u32 PTC_adc_val = 0,NTC_adc_val = 0;
u8 adc_test_std = 0;
...
u16 temp_calc ( u32 PTC_VAL,u32 NTC_VAL )
{
	u16 i = 0;
	u16 basi_tmp = 25;
	float u1 = 0;
	...
	if ( adc_test_std == 2 )
	{
		PTC_adc_val = sqrt ( PTC_adc_val/50 );
...
void temperature_handle ( void )
{
	...
	if ( adc_test_std == 1 )
	{
		...
		if ( ++adc_test >= 50 )
		{
			...
			PTC_adc_val = PTC_adc_val + ( adc_val1*adc_val1 );
